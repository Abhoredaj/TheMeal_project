...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function showFavoriteMeals() {
    ...
    if (favoritesList.length === 0) {
      ...
      const favoriteMealsHTML = favoriteMealsOnPage.map(meal => createFavoriteMealCard(meal)).join('');
  ...
  function createFavoriteMealCard(meal) {
    return `
      <div class="col-md-4 mb-4">
        <div class="card h-100" style="background-color:#e4dcf1">
          <img src="${meal.strMealThumb}" class="card-img-top" alt="${meal.strMeal}">
          <div class="card-body d-flex justify-content-between">
            <h5 class="card-title" style="font-family: 'Borel', cursive">${meal.strMeal}</h5>
          </div>
          <div class="card-footer" style="background-color:#2e003e">
            ...
            <a href="meal.html?id=${meal.idMeal}" class="btn-secondary view-details" data-meal-id="${meal.idMeal}" data-toggle="tooltip" title="View Details">
  ...
  favoriteMealsContainer.addEventListener('click', (event) => {
    ...
    if (removeBtn) {
      ...
      const isConfirmed = confirm(`Are you sure you want to remove ${mealName} from your favorites?`);
